...
void midiClass::pitchBend(uint8_t channelNo, uint8_t lsb, uint8_t msb) {
	serialPort->write(0xe0 | channelNo);
	serialPort->write(lsb);
	serialPort->write(msb);
}

void midiClass::pitchBend(uint8_t channelNo, uint16_t data) {
	pitchBend(channelNo,data & 0x7f,(data >> 7) & 0x7f);
}
